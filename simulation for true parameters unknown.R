...
Simfunction <- function(ns){
  ...
  #Method_result(with true \mu and Sigma unknown): 1-AdaBDP, 2-AdaMDP
    ...
    for (cc in 2:2){ #1:3 for three contamination ratios
      ...
      for (mm in 1:1){ #1:2 for two sparse settings
        ...
        for (aa in 1:2){ #1:2 for two FDR levels
          ...
          FDP[2, aa, cc, mm, pp] <- length(setdiff(which(result[[1]]==1), which(ou==1)))/max(1, sum(result[[1]]))
          TDP[2, aa, cc, mm, pp] <- 1-length(setdiff(which(ou==1), which(result[[1]]==1)))/n_ou
          
          FDP[1, aa, cc, mm, pp] <- length(setdiff(which(resultB[[1]]==1), which(ou==1)))/max(1, sum(resultB[[1]]))
          TDP[1, aa, cc, mm, pp] <- 1-length(setdiff(which(ou==1), which(resultB[[1]]==1)))/n_ou
        }
      }
    }
  }
...
#---------------------------------------------------------------------------------------------------------
# save(FDR, TDR, result, file="Model a_newcontami_sparsek10rho0.5.RData")
# result_a <- load("C:/Users/compl/Desktop/AdaBDP/Model a_newcontami_sparsek10rho0.5.RData")
# result_aFDR <- eval(parse(text = "FDR"))
# result_aTDR <- eval(parse(text = "TDR"))
# round((result_aFDR)*100, 2)
# round((result_aTDR)*100, 2)

# save(FDR, TDR, result, file="Model b_diffpk11.RData")
# result_b <- load("C:/Users/Ada/Desktop/AdaBDP/Model b_diffpk11.RData")
# result_bFDR <- eval(parse(text = "FDR"))
# result_bTDR <- eval(parse(text = "TDR"))
# round((result_bFDR)*100, 2)
# round((result_bTDR)*100, 2)

# save(FDR, TDR, result, file="Model c_diffpk11.RData")
# result_c <- load("C:/Users/Ada/Desktop/AdaBDP/Model c_diffpk11.RData")
# result_cFDR <- eval(parse(text = "FDR"))
# result_cTDR <- eval(parse(text = "TDR"))
# round((result_cFDR)*100, 2)
# round((result_cTDR)*100, 2)
# 
# save(FDR, TDR, result, file="Model d_newcontami_sparsediffk.RData")
result_d <- load("C:/Users/david/Desktop/AdaBDP/Model d_newcontami_sparsek10rho0.5.RData")
